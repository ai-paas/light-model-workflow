...
class OptimizeService:
    ...
    def pruning(self, optimizer: OptimizerInfo, optimize_form: ReqOptimizeWithNameAndArgsBody):
        """
        Pruning 경량화 작업
        Args:
            optimize_form: 경량화 폼
        ...
        self._save_task_info(task_info, result)

        return result

    def tensorrt(self, optimizer: OptimizerInfo, optimize_form: ReqOptimizeWithNameAndArgsBody):
        """
        TensorRT 최적화 작업
        Args:
            optimize_form: 최적화 폼
    ...
    def openvino(self, optimizer: OptimizerInfo, optimize_form: ReqOptimizeWithNameAndArgsBody):
        """
        OpenVINO 최적화 작업
        Args:
            optimize_form: 최적화 폼
    ...
    def sklearn_onnx(self, optimizer: OptimizerInfo, optimize_form: ReqOptimizeWithNameAndArgsBody):
        """
        sklearn-onnx 최적화 작업
        Args:
            optimize_form: 최적화 폼
    ...
    def npu(self, optimizer: OptimizerInfo, optimize_form: ReqOptimizeWithNameAndArgsBody):
        """
        NPU 최적화 작업
        Args:
            optimize_form: 최적화 폼
    ...
    def optimize_facade(self, optimizer_id: int, optimize_form: ReqOptimizeWithNameAndArgsBody):
        """
        최적화 작업 파사드
        Args:
            optimizer_id: 최적화 도구 id
            optimize_form: 최적화 요청 form
        Returns:
            task_info: 최적화 작업 정보
        """
        optimizer = self.optimizer_info_repo.get_optimizer_info_by_id(optimizer_id)
        ...
        if optimizer.optimizer_name == SupportOptimize.PRUNING.value:
            return self.pruning(optimizer, optimize_form)
        elif optimizer.optimizer_name == SupportOptimize.TENSORRT.value:
            return self.tensorrt(optimizer, optimize_form)
        elif optimizer.optimizer_name == SupportOptimize.OPENVINO.value:
            return self.openvino(optimizer, optimize_form)
        elif optimizer.optimizer_name == SupportOptimize.SKLEARN_ONNX.value:
            return self.sklearn_onnx(optimizer, optimize_form)
        elif optimizer.optimizer_name == SupportOptimize.NPU.value:
            return self.npu(optimizer, optimize_form)
        else:
            raise HTTPException(status_code=400, detail="Invalid optimizer name")
